...
for metric, ax, color in zip(metrics, axes, prop_cycle.by_key()['color']):
    ...
    ax.plot(mean_eval["n_datasets"].unique(), mean_eval.groupby("n_datasets").mean()[metric], "--o", markersize=10, label=f"{metric} mean over selections", color=color)
...
legend_3 = ax_3.legend(handles=[plt.Line2D([0], [0], marker='o', color='grey', markerfacecolor='grey', markersize=10, label="score per selection"),
                                plt.Line2D([0], [0], marker='o', color='grey', markerfacecolor='grey', markersize=10, linestyle="--", label="mean over selections")], loc='upper left', bbox_to_anchor=(1.05, 0.5))
