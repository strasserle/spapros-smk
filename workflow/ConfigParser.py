...
class ConfigParser():
    ...
    def _convert_pseudo_params_to_param(
        ...
        # Update or copy evaluation parameters
        if "evaluations" in config.keys():

            new_config_e = {}
            old_config_e = config["evaluations"]

            # Iterate over each batch of the configuration
            for batch in old_config_e:
                new_config_e[batch] = {}

                # Copy everything except of dataset params (because right now, only there can be pseudo parameters)
                for key in old_config_e[batch]:
                    if key != "dataset_param":
                        new_config_e[batch][key] = old_config_e[batch][key]

                # Update or copy dataset parameters of evaluations
                if "dataset_param" in old_config_e[batch]:
                    new_config_e[batch]["dataset_param"] = {}

                    for param in old_config_e[batch]["dataset_param"]:
                        # Raise an error if a parameter is given that should be set via its pseudo parameter
                        if param in target_params["dataset"]:
                            raise ValueError(
                                f"Parameter {param} ({batch}, dataset) can not be set directly, \
                        ...
                        # Convert pseudo parameters
                            ...
                            new_config_e[batch]["dataset_param"].update({
                                PSEUDO_PARAM_TO_PARAM["dataset"][param]: self._dataset_pseudo_parameter_conversion(
                                    param, old_config_e[batch]["dataset_param"][param]
                                )
                            })
                        # Copy parameters that are not converted
                        else:
            ...
            # Update the config dictionary
            config["evaluations"] = new_config_e
